...
function modify_rdp_gateway_configs() {
  if [ "${CURRENT_MAJOR_VERSION}" == "1" ] && [ "${CURRENT_MINOR_VERSION}" -ge "16" ] ; then
    ${SCRIPT_PATH}/bin/utils/yq_${ARCH} -i '.rdp-gateway.id = load("/opt/kasm/'${CURRENT_VERSION}'/conf/app/passthrough.app.config.yaml").rdp-gateway.id' /opt/kasm/${KASM_VERSION}/conf/app/passthrough.app.config.yaml
    ${SCRIPT_PATH}/bin/utils/yq_${ARCH} -i '.api.auth_token = load("/opt/kasm/'${CURRENT_VERSION}'/conf/app/passthrough.app.config.yaml").api.auth_token' /opt/kasm/${KASM_VERSION}/conf/app/passthrough.app.config.yaml
    ${SCRIPT_PATH}/bin/utils/yq_${ARCH} -i '.api.public_jwt_cert = load("/opt/kasm/'${CURRENT_VERSION}'/conf/app/passthrough.app.config.yaml").api.public_jwt_cert.[0]' /opt/kasm/${KASM_VERSION}/conf/app/passthrough.app.config.yaml
...
function modify_rdp_https_gateway_configs() {
  if [ "${CURRENT_MAJOR_VERSION}" == "1" ] && [ "${CURRENT_MINOR_VERSION}" -ge "16" ] ; then
    ...
    ${SCRIPT_PATH}/bin/utils/yq_${ARCH} -i '.api.public_jwt_cert = load("/opt/kasm/'${CURRENT_VERSION}'/conf/app/rdp_https_gateway.app.config.yaml").api.public_jwt_cert' /opt/kasm/${KASM_VERSION}/conf/app/rdp_https_gateway.app.config.yaml
    ...
    ${SCRIPT_PATH}/bin/utils/yq_${ARCH} -i ".api.port = ${RDP_HTTPS_GW_API_PORT}" /opt/kasm/${KASM_VERSION}/conf/app/rdp_https_gateway.app.config.yaml
  fi
